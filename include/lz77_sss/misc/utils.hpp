...
uint64_t time_diff_sec(std::chrono::steady_clock::time_point t1, std::chrono::steady_clock::time_point t2)
{
...
inline char_t unsigned_to_char(pos_t c)
{
    static_assert(std::numeric_limits<pos_t>::min() == 0);
    static_assert(sizeof(char_t) == 1);
    return *reinterpret_cast<char_t*>(&c);
}
...
using ith_type = typename std::tuple_element_t<i, std::tuple<Ts...>>;
